...
device_affinity_t ui_devices[] = {
  ...
  {&BLE_device, 0},
#if defined(TX_BLE_MSP)
  // Affinity 1 (loop core) is load-bearing once the CRSF bridge lands: the
  // pump will call crsfRouter.processMessage()/TXOTAConnector::forwardMessage(),
 ...
 *
 * Caller must clear commitInProgress once the change is done. Shared by the
 * config commit and the BLE MSP session-rate hop -- both need the radio idle
 * and off the SPI bus, and neither should be the only place that knows it.
 */
...
static void UpdateSessionRate()
{
  ...
  // rather than the configured rate. Without this a session is unusable:
  // every data uplink requests a sync packet, and each would name the
  // configured rate while the radio ran elsewhere -- the receiver follows it,
  // finds nothing, hunts, reacquires, and is sent away again.
  ...
  // Warn the receiver on the rate it is still listening on. This only says
  // anything because sessionRate is published above; the sync packet reads
  // the intended rate.
  if (!receiverWarned)
  {
