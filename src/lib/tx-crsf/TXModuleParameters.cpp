...
// state. Its info line doubles as the bench readout (see BleMspStatus): the
// serial log only reaches the backpack UART, so with the module seated in
// the radio this is the one place a test result can actually be read.
...
void TXModuleEndpoint::handleWifiBle(propertiesCommon *item, uint8_t arg)
{
  ...
  else if ((void *)item == (void *)&luaBLEMsp)
  {
    ...
    textRunning = BleMspStatus(); // live: "Adv w3" / "Conn+ w12 r11"
  ...
  switch ((commandStep_e)arg)
  {
    ...
    case lcsConfirmed:
      if (liveStatus)
      {
        ...
        // so the refusal is already there by the time we send.
        setTargetState();
...
void TXModuleEndpoint::registerParameters()
{
  ...
  registerParameter(
      &luaBleMspShaping, [this](propertiesCommon *item, uint8_t arg) {
          ...
          // config, which raises a device event, and updateParameters()
          // (subscribed to EVENT_ALL) then republishes all the values. Shaping
          // is deliberately not persisted so it raises no event, leaving the
          // handset's read-after-write to return the stale 0 -- the selector
