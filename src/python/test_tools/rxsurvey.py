# Host-side watcher for the in-flight passive RF survey (-DDEBUG_RF_SURVEY).
# Talks to the receiver's CRSF UART over BetaFlight serial passthrough and
# decodes the survey's 3-byte transport riding the link-statistics downlink
# bytes; unless --no-stream it also requests the full-fidelity 0x83 bench
# stream and checks every deduped sample appears identically on both.
# Arm from the handset Lua "RF Survey" option or remotely with --mode; the RC
# link must be up. Levels are antenna-referred but uncalibrated.
#
...
# bootloader.py (in src/python) is stdlib-only; pyserial is imported where
# used, so --selftest runs anywhere, including CI, with nothing installed
...
def open_rx_serial(args):
    ...
    # DTR and RTS deasserted before open: on an STM32 FC those are reset lines,
    # and pyserial raising them would reboot the FC out of serialpassthrough
    s = serial.Serial()
    s.port = args.port
...
# The bench command ('sf', RXEndpoint::handleRaw): appended bytes are
# [stream on/off, optional mode 0=Off 1=Both 2=900 3=2.4]
...
GATE_TEXT = [
    (FLAG_GATED_RADIO, "the radio is not running -- receiver is in WiFi/BLE mode. "
                       "Power-cycle it; the transmitter is not the problem"),
    (FLAG_GATED_LINK, "no RC link (the survey needs the transmitter ON)"),
...
# The link-statistics frame; the survey rides its last three (downlink) bytes.
...
# Port of SurveyDecodeFrame: a dict, or None if not a well-formed survey frame.
def decode_survey_payload(payload):
    ...
    return {
        ...
        # chan2 joins against this axis when present; 0 on a single-band link
...
# Status (sub-type 0x04), sent after every bench command and while streaming.
...
# Radio-2 frequency: the second band's axis when present, the primary otherwise.
...
# The 3-byte transport (SurveyPackDebug/Unpack): magnitudes are -dBm (127 =
# none); bit 7 is toggle / clean / assignment-or-band respectively.
DBG_MAG_INVALID = 127
DBG_CHAN_NONE = 127
...
# One chunk of serial data -> the per-frame handlers.
...
# enum_rate values of the dual-band rates (RATE_LORA_DUAL_* in common.h, pinned
# at 100): debug[2] bit 7 is a band bit on these, an assignment bit otherwise.
DUAL_BAND_RF_MODES = {100, 101}


# Watch the 3-byte transport live, deduped on the freshness toggle, and (unless
# --no-stream) cross-check every sample against the full-fidelity 0x83 stream.
def run_watch(args):
...
# Captured from SurveyEncodeFrame() compiled for the host, so the decoder is
# checked against the real encoder; regeneration recipe in test/test_rxsurvey/README.
GOLDEN_FRAMES = [
...
# From SurveyPackDebug(), same recipe: an ordinary sample, then the sentinels.
...
def run_selftest():
    ...
    # the axis must reproduce the FHSS grid exactly
    ...
    # GATED_RADIO must be reported before the GATED_LINK it causes
    ...
    # a foreign sub-type on the shared 0x83 frame is rejected, not crashed on
    ...
    # positivized RSSI must come back negative, or a -80 dBm link reads as +80
