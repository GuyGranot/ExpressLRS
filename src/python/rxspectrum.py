...
# is printed as text.
#
# Levels are uncalibrated (receiver chain and antenna dependent): comparable
# within one scan, not across receivers or against a lab instrument.
#
...
# pyserial and the passthrough helpers are imported where used, so --selftest
# and --replay run anywhere, including CI, with nothing installed
...
# Trigger sequence in bootloader.py's non-CRSF command form, handled by
# RXEndpoint::handleRaw; appended bytes select band/rbw/compare
...
# Sensing bandwidth, resolved per band by the receiver; each step halves Wide
...
# Bar baseline: one below the -127 a real reading clamps to, so a floored bin
# shows as a hairline
PLOT_FLOOR_DBM = -128
...
# Port of SpectrumDecodeFrame: a dict, or None on a malformed or foreign frame
    ...
    def update(self, dec):
        ...
        if b is None or b["total"] != n or b["start"] != dec["startFreqKhz"]:
            ...
            b = {
                ...
                "live": [None] * n,
                "max": [None] * n,
                # geometry is a function of start/step/total alone; a change to
                # any of them replaces this dict
