...
// The survey's channel axis must land on the real FHSS grids exactly, or a
// per-channel join against a spectrum sweep compares different frequencies.
...
// so a frame carries a second axis: chan1 joins against the primary grid and
