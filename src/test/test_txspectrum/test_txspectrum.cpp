...
// asserted by a static_assert in devTxSpectrum.cpp, which fires on every
// firmware build rather than only in this suite.
...
{
    // Comparing antennas puts two traces on ONE axis, so the axis stops being
    // enough to tell them apart and RADIO_2 becomes part of a trace's identity.
    // A decoder that keys on the axis alone blends an antenna pair into one
    // trace, which looks like a plausible measurement and is not one.
    int8_t in[2] = {-90, -91};
    ...
    // Same axis, same trace id, same bins: the flag is the only difference, so
    // it is the only thing a consumer can key on.
...
void test_payload_size_is_pinned(void)
{
    ...
    // against EdgeTX's 255-byte Lua queue (~4 frames). That 61 fits a CRSF
    // frame is checked by the static_assert in SpectrumSweep.cpp.
...
{
    // Pins byte order, which a roundtrip test cannot: elrs.lua reads these with
    // fieldGetValue(), a BIG-endian reader, and a flip renders silently wrong.
    int8_t in[2] = {-90, -91};
    ...
    // The sensing bandwidth rides along so a trace can be read back later:
    // two traces of a band taken at different bandwidths are not comparable.
...
void test_golden_vector_frame_is_pinned(void)
{
    ...
    // 55 payload bytes + the 2 ext-header addresses = the 57-entry table EdgeTX
    // hands Lua, whose last index is data[57] -- exactly the highest index
...
{
    // 80 is the largest freq_count in any domain table (FHSS.cpp); a bigger
    // domain would silently truncate at BeginScan()'s clamp. A per-band bound:
    // one band is scanned per screen, so a combined plot (which would need 120)
    // cannot exist.
...
void test_decode_rejects_malformed(void)
{
    ...
    static const struct
    {
        ...
        // The frame type is shared, so foreign sub-types must be refused rather
        // than parsed as spectrum data -- otherwise a future ELRS sub-protocol
        // on this type renders as a plot of its own bytes.
