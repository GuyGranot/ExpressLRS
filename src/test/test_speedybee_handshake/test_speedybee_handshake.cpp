// Golden-byte tests for the SpeedyBee BLE handshake responder.
// Vectors are now anchored to a capture of a genuine SpeedyBee F405 V4
// (python/ble-msp-emulator/capture_fc.py -> fc_handshake.txt); the Python
// implementation in sb_protocol.py and the HCI-snoop exchanges in
// dunaevai135/speedybee_ble_bridge agree with them.
//
// The response header is [cmd][0x00][varint payloadLen]. That matters: the
// earlier tests asserted a 2-byte big-endian length, which is identical to a
// varint below 128, so they passed while the 133-byte device-info reply went
// out malformed and the iOS app hung up on it. decodeHeader() below parses
// the length the way the app does, so a wrong encoding fails here first.

#include "SpeedyBeeHandshake.h"
#include <string.h>
#include <unity.h>

// Fixtures, not the shipped identity: product_name is not declared under
// UNIT_TEST (options.h), so a host test cannot use what the firmware uses.
...
// Parse [cmd][0x00][varint len] the way the app does, and check the declared
// length actually accounts for every remaining byte. Returns the payload
// offset. Any framing slip shows up here rather than on the bench.
...
void test_device_info_layout()
{
    ...
    // Real FC replies f6 00 f4 01 08 0d 1a ef 01 ... -- a 4-byte header,
    // because its 244-byte payload needs a 2-byte varint. Ours is smaller but
    // must be framed the same way.
    ...
    // Captured layout, our identity in it. Offsets are load-bearing: the app
    // reads fields at fixed positions, so a product code of the wrong length
