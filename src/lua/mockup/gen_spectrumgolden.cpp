/*
 * Generates spectrumgolden.lua: golden ELRS_VENDOR frames for the simulator
 * harness, produced by the real firmware encoder (SpectrumEncodeFrame) -- the
 * bytes a TX actually emits, not a re-reading of the spec, which is what lets
 * the simulator prove the encoder and elrs.lua's hand-rolled decoder agree.
 * Regeneration recipe: README.md in this directory.
 *
 * SpectrumProtocol.h is <stdint.h>-only, so this builds with bare g++ and is
 * never compiled into any firmware or test target.
 */
