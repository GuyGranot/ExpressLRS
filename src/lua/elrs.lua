...
-- Renders sub-type 0x01 of CRSF_FRAMETYPE_ELRS_VENDOR (SpectrumProtocol.h,
-- offset by 2: EdgeTX keeps the ext-header addresses, so payload[0] is data[3]).
-- Inert unless such a frame arrives, so stock firmware is fine.
local spectrum = nil
...
local function specY(v)
  ...
  -- B&W drawLine silently draws nothing when a coord exceeds LCD_W/H; clamp first
...
local function parseSpectrumMessage(data)
  ...
  -- count), and comparing puts two traces on one axis, so source keys it too
  ...
  if s == nil or s.total ~= total or s.startKhz ~= startKhz or s.source ~= source then
    ...
    local fieldId
    if s == nil then
      -- First entry needs a running popup, else post-EXIT frames re-open the plot
    ...
    -- Pre-built title keeps the redraw allocation-free
  ...
  -- Redraw once per completed trace (~100ms), not per frame
...
local function drawSpectrum(s)
  ...
  -- elrsFlagsInfo is what puts "[ ! Armed ! ]" on the plot when arming aborts
  ...
  -- Cursor first so it shows through the gaps above the bars
  ...
  -- string.format, not drawNumber PREC1, which is unregistered on some targets
  -- and would render "9149"
  local c = s.cursor
  local y = LCD_H - 8
...
local function runSpectrumPage(event)
  local s = spectrum
  if event == EVT_VIRTUAL_EXIT then
    -- lcsCancel: the TX reboots out; frames still in flight are refused by
    -- parseSpectrumMessage, which needs a popup that entry already nil'd
  ...
  elseif event == EVT_VIRTUAL_ENTER then
    -- Re-invoking the running Start Scan resets max-hold
  ...
  elseif event == EVT_VIRTUAL_NEXT_PAGE or event == EVT_VIRTUAL_PREV_PAGE then
    -- Next Source is registered directly after Start Scan, so its id is scan + 1
...
local function setLCDvar()
  ...
  if (lcd.RGB ~= nil) then
    ...
    textYoffset = 3
    textSize = 8
    -- B&W only: SPEC_Y1 stays nil on colour, which refuses the spectrum view
    SPEC_Y0 = SPEC_BAR + 2
...
local function setMock()
  ...
  -- Fake TX replaying encoder golden frames through this script's decoder;
  -- rebinds crossfireTelemetryPop/Push so it must load after the fields above
