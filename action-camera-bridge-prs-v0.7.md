# Action Camera Bridge — Product Requirements Specification v0.7

**Status:** Draft
**Supersedes:** v0.6
**Target platforms:** Betaflight 2025.12+, INAV 8.x
**Behavioural baseline:** Betaflight **2025.12.5**, INAV **8.0.1** (§0)
...
v0.6's architecture is **retained unchanged**: the bridge observes configured RC intent,
never interprets FC armed state, defaults Record to a level control for recoverability, and
lets Betaflight and INAV use deliberately different RX-loss mechanisms. Nothing in this
revision reopens those decisions.

v0.7 fixes the *control path around* those decisions, and repairs a systematic defect in how
this document cites its sources.
...
`iNavFlight/inav` at tag `8.0.1`. Every line number in this document was **read from those
tags on 2026-08-25**. Forward compatibility against `master` is a separate audit and its
findings shall be recorded as version notes, never by silently re-pointing a citation.
...
##### 12.4.3 Startup qualification — required

The bridge's control input has an explicit qualification state, and it starts unqualified:
...
control input = UNQUALIFIED          ← no camera-control transitions of any kind
    ↓
observe one valid guarded RC snapshot (§12.4.1)
    ↓
...
This replaces v0.6's "startup assertion" requirement, which assumed the guard is already
asserted when the bridge first reads `MSP_STATUS`. **That assumption is false**, and the
window is wide enough to hit routinely:
...
- The flag is asserted later, by one of: `failsafeOnValidDataReceived()` on the **first**
  ...
  `failsafeCheckDataFailurePeriod()` (`:212-222`, set at `:219`), which the scheduler runs
...
- qualification shall not be inferred from elapsed time, from the FC being detected, or from
  a guard state observed before any RC sample was bracketed;
- **every** later re-qualification — after an RX-loss freeze, after a bridge reboot, after an
  FC reboot — follows the same rule: re-baseline, reconcile level controls, emit no edge.
...
that has been running for ten minutes. v0.6's framing covered only the first case.
...
Separately, the `UNQUALIFIED` startup state of §12.4.3 raises **no** RC-loss warning: there
is no accepted intent yet to compare against, so no camera-health claim is being suppressed.
...
- **bridge powered up while the FC has not yet validated RC data**, including the ~1.5 s
  window in which the Betaflight guard still reads clear (§12.4.3 startup qualification);
...
| **Startup qualification: no camera-control transition of any kind occurs before the first guarded RC snapshot; the first qualified sample reconciles a level control and emits no edge (§12.4.3)** | Required | Required |
| **Startup, no-receiver case: with the transmitter off, no spurious record occurs during the ~1.5 s window before the guard asserts, for a control range spanning `midrc` (§12.4.3)** | Required | n/a |
...
| **Bridge powered up simultaneously with the FC, transmitter ON** | input stays `UNQUALIFIED` until the first guarded snapshot; no RC-loss warning; first qualified sample reconciles level controls and emits no edge (§12.4.3) |
| **Bridge powered up simultaneously with the FC, transmitter OFF, Record range spanning `midrc`** | no recording starts during the ~1.5 s pre-assertion window; qualification never completes on init values (§12.4.3) |
| **Bridge attached to an FC that has been running for minutes** | identical behaviour to the two rows above; qualification is not a boot-timing assumption (§12.4.3) |
...
| Camera stops recording on its own (card filled) while the §12.4 guard is frozen | `REC STOPPED` **is raised**, compared against the last accepted intent (§18) |
...
report the others as defects. The three **startup** rows exist because v0.6 assumed a guard
state that Betaflight does not actually guarantee, and the transmitter-OFF row is the one
that would have caught it. The sustained reconnect row is the direct test of §3.3 and shall
not be waived.
...
at source — and with it the guarded-commit bracket (§12.4.1), the recovery-window semantics
(§12.4.2) and the startup-qualification rule (§12.4.3). All three exist only because an
arming-permission flag is being borrowed as an input-validity signal and read through a
transport that cannot deliver the two together. That is a property of that architecture, not
...
7. **Betaflight guard — startup qualification.** Three runs (§12.4.3):
   **(a)** bridge and FC powered together with the transmitter **on**;
   **(b)** the same with the transmitter **off** and a Record range deliberately configured to
   span `midrc`, held for at least 5 s — no recording shall start at any point, in particular
   not during the ~1.5 s before the guard asserts;
   **(c)** bridge attached to an FC that has been running for several minutes.
   In all three, confirm no camera-control transition before the first guarded snapshot, no
   RC-loss warning for the unqualified state, and that the first qualified sample reconciles
   level controls without emitting an edge.
8. **Betaflight guarded commit.** Induce RX loss repeatedly while polling, with
...
9. **`Combi` arbitration.** Walk the §12.1.1 truth table on hardware, including that a
...
10. **Range evaluator.** Confirm ±10 µs hysteresis at a boundary using a slowly swept
    channel — the control shall not chatter — and that setup rejects a 30 µs range and a pair
    of ranges 15 µs apart (§12.2.1).
11. **INAV 8:** RX loss leaves the camera AUX returned by `MSP_RC` at its last-good value,
...
12. **`REC STOPPED` during a freeze.** With the guard frozen and last accepted intent
    `RECORD`, stop the camera externally (fill the card, or power it down) and confirm the
    warning **is** raised against the frozen intent (§18).
13. Custom message / custom element writes are accepted and rendered while armed on both
    Betaflight and INAV. This validates in-flight availability of the OSD path; it does not
    create an armed-state dependency.
14. Correct rendering on analog and on MSP DisplayPort.
15. Four fields independently updateable.
16. No flash write observed across a long operating session (verify by configuration
    persistence check across power cycle, not by inference).
17. Graceful degrade confirmed on an INAV target built without `USE_PROGRAMMING_FRAMEWORK`,
    and on a Betaflight build older than 2025.12.
18. **Baseline conformance.** Run the full set against stock **Betaflight 2025.12.5** and
    **INAV 8.0.1** specifically (§0). A pass on a newer build is evidence about that build,
    ...
    the class of defect a master-only test run would have missed.
