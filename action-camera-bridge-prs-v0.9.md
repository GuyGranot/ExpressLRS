# Action Camera Bridge — Product Requirements Specification v0.9

**Status:** **Final — implementation baseline.** Requirements are frozen; further refinement belongs in design/implementation documents
**Supersedes:** v0.8
...
Three edits, all closing gaps between a stated rule and the section it points at, plus a
fourth applied at freeze. No architectural change.

| Change | Rationale | Section |
| --- | --- | --- |
| **The §0 version contract is now enforced.** A new version floor runs after FC identification and **before** capability probing; a build below the minimum enters `UNSUPPORTED_FC_VERSION`, in which no camera-control input is acted on and no platform OSD write is issued | §0 declared 2025.12.0–.4 and 8.0.0 unsupported and nothing implemented that. §5.1 admitted any Betaflight ≥ 2025.12, and admitted INAV on the capability probe alone — so an 8.0.0 target, whose probe succeeds, ran the full backend. §27.1 already demanded those builds "degrade rather than half-work" | §5.1, §9, §10.3, §24.1, §27.1 |
| **§5.2's armed-state rule weakened to one that is actually true**, and the false claim called out rather than narrowed away | v0.8 stated that no camera behaviour anywhere differs by armed state, which §12.4.4 contradicts — an INAV bridge rebooting while armed stays `UNQUALIFIED` until disarm. What is invariant is the control *mapping*, not the timing of when the bridge starts acting | §5.2, §24.1 |
| **§21.1 corrected to "no steady-state arm-state poll *after qualification*"** | v0.8's "no runtime arm-state poll" forbade the one behaviour §12.4.4 requires: a bridge that reboots at altitude on INAV must keep polling `MSP2_INAV_STATUS` in flight until it can qualify | §21.1, §24.1 |
| **Condition B's `margin` made normative:** `max(2 × qualification poll period, 100 ms)` | "A small number of poll intervals" is not a requirement anything can be tested against. Two poll periods guarantee two sampling opportunities against a sticky assertion; the floor covers MSP round-trip latency | §12.4.3, §24.1, §27.1 |
| **Condition B continuity measured in observations, not wall time** | A bridge whose status polls are all timing out would otherwise qualify by letting the clock run, having observed nothing. Timeouts extend the interval; an interval with no successful observation cannot qualify at all | §12.4.3, §24.2, §27.1 |

### Carried forward from v0.8

The architecture is **retained unchanged**, as it was in v0.7: the bridge observes configured
RC intent, never uses FC armed state as a camera-control or camera-policy input, defaults Record to a level control for
...
Read from the same tags on **2026-08-26**, supporting the §5.1 version floor:

| Citation | What it establishes |
| --- | --- |
| [BF] `msp/msp.c:642-647` | `MSP_FC_VERSION` sends `FC_VERSION_YEAR - FC_CALVER_BASE_YEAR`, month, patch, then a Pascal-string version — the first byte of a 2025 release is **25**, and the payload is longer than INAV's |
| [BF] `build/version.h:28-35` | `FC_CALVER_BASE_YEAR 2000`; `2025 / 12 / 5` for the pinned tag. Betaflight 4.5 and earlier put `4` in the same byte, so the byte triple stays well-ordered across the semver → calver transition |
| [INAV] `fc/fc_msp.c:369-373` | `MSP_FC_VERSION` sends plain `MAJOR / MINOR / PATCH` with no trailing string — `8 / 0 / 1` on the pinned tag |
| [BF] `msp/msp.c:4080-4090` | the `MSP2TEXT_CUSTOM_MSG_*` cases carry **no** `USE_OSD` guard; only the craft/pilot-name re-analysis at `:4101-4104` is guarded. Acceptance of a custom-message write is therefore not evidence that the target renders an OSD — which is why Betaflight has no runtime capability probe |
...
> camera-control input shall be acted on and no platform-specific OSD write shall be
> issued. Identification, setup, diagnostic and firmware-update functions remain available,
> and the state shall be surfaced to the user (§16) with the detected and required versions.
...
`armingFlags` bit position — is read from the pinned baselines and is verified for those
tags only (§0). Acting on control input from an unverified build means acting on a control
path whose failsafe semantics have not been checked, which is the precise defect §0 exists
to prevent. A bridge that does nothing is a correct bridge; a bridge that records on an
unverified failsafe path is not.

**What is compared.** Both platforms answer `MSP_FC_VERSION` with three unsigned bytes.
The bridge shall compare that byte triple lexicographically against the floor and shall
**not** parse the version string.
...
- **INAV:** issue `MSP2_INAV_CUSTOM_OSD_ELEMENTS` (0x2100). The FC replies with
  ...
  may undefine it.
- **Betaflight:** there is no runtime capability probe, and none is required. On a build
  that has passed the version floor the custom-message slots are unconditional: the
  `MSP2_SET_TEXT` handler writes `pilotConfig.message[]` with no `USE_OSD` guard on that
  case ([BF] `msp/msp.c:4080-4090`). Acceptance of the write is therefore **not** evidence
  that the target renders an OSD at all; that is confirmed once on the bench (§27.1), not
  at runtime. The version floor is the whole programmatic test on this platform.
...
Accordingly, no `MSP_BOXIDS` ARM lookup is part of the V1 runtime contract.

**The rule, stated so that it survives §12.4.4:**
...
The distinction that matters is between an input and a trust gate:
...
**Be honest about what this costs.** v0.8 claimed here that "no camera behaviour anywhere in
this document differs according to whether the aircraft is armed", and that claim was false —
§12.4.4 makes an INAV bridge that reboots while armed stay `UNQUALIFIED` until disarm, which
is observably different behaviour. A rule refuted by the section it points at is the same
defect this document has now corrected twice (v0.7's §12.4.3, v0.6's startup assertion), and
it is not fixed by narrowing the exception until the sentence survives. It is fixed by
stating the weaker rule that is actually true.

What remains invariant, and what §24.1 tests, is the mapping itself: a given control in a
given range produces the same camera action regardless of armed state. Armed state can delay
the bridge starting to act. It can never change what acting means.
...
**Minimum version:** Betaflight **2025.12.5** (§0), enforced by the §5.1 version floor
before any write in this section is issued. The custom-message feature merged 2024-12-22 to
the 2025.12 milestone, but feature availability is not the contract: the supported floor is
the .5 patch release, which is also the citation baseline. 2025.12.0 through .4 carry the
feature and are still `UNSUPPORTED_FC_VERSION`.
...
**Minimum version:** INAV **8.0.1** (§0), within the 8.x family, enforced by the §5.1
version floor before any write in this section is issued. INAV 8.0.0 answers the capability
probe successfully and is nonetheless `UNSUPPORTED_FC_VERSION` — the probe reports what the
target was built with, never whether the build is inside the support contract. INAV support
is a release-blocking requirement, not an optional compatibility mode.
...
§5.1); no custom target; no firmware patch; no
...
2. **`margin` exists to guarantee the assertion is *observed*, not to guarantee it has
   ...
   **`margin = max(2 × qualification poll period, 100 ms)`** — normative, not advisory. v0.8
   left it as prose ("a small number of poll intervals"), which is not a requirement anything
   can be tested against. The derivation:

   - the assertion is **sticky**, re-asserted every 10 ms while data is absent ([BF]
     `flight/failsafe.c:212-221`), so it does not have to be caught in a narrow window — it
     only has to be sampled once while it stands;
   - the interval in which it stands but B has not yet expired is at least `margin` long, so
     a margin of two poll periods yields **at least two** sampling opportunities, tolerating
     one lost or failed transaction;
   - the formula scales automatically with the poll rate: 10 Hz → 200 ms, 20 Hz → 100 ms;
   - the 100 ms floor exists because MSP round-trip latency and scheduler jitter are not
     zero. At a hypothetical 50 Hz qualification poll, `2 × 20 ms` would be comparable to a
     single transaction's own latency, which is precisely the kind of unstated timing
     assumption §12.4.3 exists to remove.

   **Continuity is measured in observations, not in wall time.** A bridge whose status polls
   are timing out is not observing anything, and shall not qualify by letting the clock run:
   each unanswered or failed status transaction during a condition-B interval extends that
   interval by one poll period. An interval containing no successful observation cannot
   qualify at all, whatever its duration.
...
**There is no steady-state arm-state poll after qualification.** That is the accurate form of
the rule, and v0.8's "no runtime arm-state poll" was not: a bridge that reboots at altitude on
INAV *must* keep issuing `MSP2_INAV_STATUS` during flight until it sees a trustworthy
disarmed state with `ARMING_DISABLED_RC_LINK` clear (§12.4.4). The v0.8 sentence forbade the
one behaviour §12.4.4 requires.
...
instant the input qualifies and never resumes unless the input becomes unqualified again.
Neither appears in the steady-state budget below. An implementation still issuing either
**after qualification** has misread §5.2; one issuing them *before* qualification, in flight,
...
- **`MSP_FAILSAFE_CONFIG` unreadable or unsupported**, removing condition B entirely
  (§12.4.3);
- **status polls timing out through a qualification interval**, so that wall-clock time
  passes without any observation being made (§12.4.3);
...
7. **Betaflight guard — startup qualification.** Six runs (§12.4.3). In every one, confirm no
   ...
   **(c)** Bridge attached to an FC running for several minutes — expect condition **B**.
   Timestamp the bridge's first MSP exchange and the qualification instant, and assert that
   the measured interval is **≥ the computed `grace`**, with `margin = max(2 × qualification
   poll period, 100 ms)`. Repeat at 10 Hz and 20 Hz qualification polling: the observed
   `grace` shall differ by the margin term. A qualification that lands early is a failure even
   though nothing visibly misbehaves — it means the interval is not being computed.
   ...
   **(g)** Repeat (c) while dropping a defined fraction of `MSP_STATUS` replies. Qualification
   shall be delayed by one poll period per unanswered transaction, and a run in which every
   status poll times out shall never qualify however long it is left — the clock alone does
   not qualify anything (§12.4.3).
...
20. **Degrade paths, both platforms and both kinds.** Confirm the two degraded states are
    reached independently and behave differently (§5.1):
    ...
    b. *Version degrade, Betaflight.* **2025.12.4** — a build that carries the custom-message
       feature and would otherwise half-work — enters `UNSUPPORTED_FC_VERSION`: no OSD
       writes **and** no camera control, with the reason shown to the user.

    c. *Version degrade, INAV.* **8.0.0** — a build whose capability probe succeeds — enters
       `UNSUPPORTED_FC_VERSION` on the same terms. This is the case that fails if the
       implementation probes before it gates.

    d. *Ordering.* In both (b) and (c), confirm by MSP capture that no camera-control input
       was acted on at any point, including during the interval between identification and
       the gate decision.
